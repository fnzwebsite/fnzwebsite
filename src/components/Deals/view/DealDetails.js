        ...
        if (this.props.dealData) {
            LoadRows = Object.keys(self.props.dealData).sort((a, b) => b.name - a.name).map(function (keyName, keyIndex) {
                return <tr>
                    ...
                    <td>{convertCurrency(self.props.dealData[keyIndex].subscriptions)}</td>
                    <td>{convertCurrency(self.props.dealData[keyIndex].redemptions)}</td>
                    <td>{convertCurrency(self.props.dealData[keyIndex].netFlow)}</td>
                    <td>{convertCurrency(self.props.dealData[keyIndex].roundedPrice)}</td>
                    <td>{self.props.dealData[keyIndex].priceDate}</td>
                </tr>
            });
    ...
    render() {
        ...
        for (var i = 0; i < isin.length; i++) {
            for (var j = 0; j < this.props.location.state.data.length; j++) {
                if (isin[i] == this.props.location.state.data[j].instrumentKey) {
                    ...
                    priceDate = this.props.location.state.data[j].priceDate;
                    if (redemptionSum < 0) {
        ...
        return (
            <div className="container-fluid">
                <div className="mt-6">
                    <div className="uk-grid uk-grid-divider uk-grid-medium">
                        <div className="uk-width-large-2-10 lt-details">
                            <h2>Calendar</h2>
                        ...
                        <div className="uk-width-large-8-10 rt-details">
                            <div className="md-card-content">

                                <Table id="dt_colVis" className="uk-table" cellspacing="0" width="100%"
                                       dealData={tableData}/>
