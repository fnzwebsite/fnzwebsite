...
import {fetchAcdPrevious} from '../actions/acdPrevious';
...
class PreviousBox extends React.Component {
    ...
    componentWillMount() {
          ...
          this.props.fetchAcdPrevious(date, localStorage.getItem('acdId'))
//          console.log(JSON.stringify(this.props.acdPrevious));
    ...
    render() {
      if (this.props.previous) {
            this.state = {
                boxData: boxDataCalculation(this.props.previous)
            }
...
const mapStateToProps = ({acdPrevious}) => {
    const {
        previous,
    } = acdPrevious;
    return {
